"""
BaseNeuralNetPCA.py — Neural network for PCA-reduced input data.
Same architecture as BaseNeuralNet but expects PCA-transformed features.
"""
...
class BaseNeuralNetworkPCA:
    """
    3-layer neural network (2 hidden + 1 output) with vanilla SGD.
    Designed for PCA-transformed inputs (smaller feature dimension).
    """
    ...
    def train(self, X_train_pca, y_train, X_val_pca, y_val, epochs=100):
        train_losses, val_losses = [], []
        train_accuracies, val_accuracies = [], []
        train_f1s, val_f1s = [], []

        for epoch in range(epochs):
            Z1, A1, Z2, A2, Z3, A3 = self.forward(X_train_pca)
            self.backward(X_train_pca, y_train, Z1, A1, Z2, A2, Z3, A3)

            train_loss = cross_entropy_loss(y_train, A3)
            train_acc = accuracy(y_train, A3)
            train_f1 = f1_score(y_train, A3)

            _, _, _, _, _, A3_val = self.forward(X_val_pca)
